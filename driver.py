...
def run(parent_dir_name: str, config: BaseConfig = default_config) -> None:
    ...
    all_files_in_parent_dir = get_files_in_dir(parent_dir, "py", config.EXCLUDE_DIRS, config.EXCLUDE_FILES)

    all_todos_items = parse_files_for_todo_items(parent_dir_name, all_files_in_parent_dir)
